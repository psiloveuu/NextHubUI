...
local function main()
	...
	local execute, clear, dumpbtn, bytecodeBtn
	
	local PreviousScr = nil
	local CurrentScr = nil
	...
	local function buildBytecodeText(scr)
		...
		if not s or type(bytecode) ~= "string" then
			text = "-- Unable to view bytecode.\n"
			if Settings.ScriptViewer.ShowMoreInfo then
				text = text .. "-- Script Path: "..getPath(scr).."\n"
				if env.getscriptbytecode == nil then
					text = text .. "-- Reason: Your executor does not support getscriptbytecode.\n"
				else
					text = text .. "-- Reason: "..tostring(bytecode).."\n"
				end
				text = text .. "-- Executor: "..executorName.." ("..executorVersion..")"
			end
		else
			text = "-- Script Path: "..getPath(scr).."\n"
			if Settings.ScriptViewer.ShowMoreInfo then
	...
	ScriptViewer.ToggleBytecode = function()
		if not CurrentScr or type(env.getscriptbytecode) ~= "function" then return end

		if BytecodeMode then
			BytecodeMode = false
			bytecodeBtn.Text = "Show Bytecode"
			dumpbtn.TextColor3 = PreviousScr and Color3.new(1,1,1) or Color3.new(0.5,0.5,0.5)
			codeFrame:SetText(ScriptText or "")
		else
			ScriptText = codeFrame:GetText()
			BytecodeMode = true
			bytecodeBtn.Text = "Hide Bytecode"
			dumpbtn.TextColor3 = Color3.new(0.5,0.5,0.5)
			codeFrame:SetText(buildBytecodeText(CurrentScr))
		end
	end
	...
	ScriptViewer.Init = function()
		...
		copy.TextTruncate = Enum.TextTruncate.AtEnd
		...
		save.TextTruncate = Enum.TextTruncate.AtEnd
		...
		dumpbtn.TextTruncate = Enum.TextTruncate.AtEnd
		...
		dumpbtn.MouseButton1Click:Connect(function()
			if PreviousScr ~= nil and not BytecodeMode then
				pcall(ScriptViewer.DumpFunctions, PreviousScr)
			end
		end)
		
		bytecodeBtn = Instance.new("TextButton",window.GuiElems.Content)
		bytecodeBtn.BackgroundTransparency = 1
		bytecodeBtn.Position = UDim2.new(0.75,0,0,0)
		bytecodeBtn.Size = UDim2.new(0.25,0,0,20)
		bytecodeBtn.TextTruncate = Enum.TextTruncate.AtEnd
		bytecodeBtn.Text = "Show Bytecode"
		bytecodeBtn.TextColor3 = Color3.new(0.5,0.5,0.5)
		
		if env.getscriptbytecode then
			bytecodeBtn.TextColor3 = Color3.new(1,1,1)
			bytecodeBtn.Interactable = true
		else
			bytecodeBtn.TextColor3 = Color3.new(0.5,0.5,0.5)
			bytecodeBtn.Interactable = false
		end

		bytecodeBtn.MouseButton1Click:Connect(function()
			ScriptViewer.ToggleBytecode()
		end)
	...
	ScriptViewer.ViewScript = function(scr)
		...
		ScriptText = nil
		BytecodeMode = false
		if bytecodeBtn then bytecodeBtn.Text = "Show Bytecode" end
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, Console, ModelViewer, SettingsWindow--[[, SecretServicePanel]], DefaultSettings, Notebook, Serializer, Lib local ggv = getgenv or nil
local API, RMD
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Console","ModelViewer","SettingsWindow"}
	Main.Elevated = false
	...
	Main.InitEnv = function()
		...
		env.isdecompile = function()
			return typeof(decompile) == "function" or false
		end

		-- DECOMPILER (built-in executor decompile only)
		local function isDecompileError(res)
			if type(res) ~= "string" then return true end
			return res:find("Failed to decode", 1, true) ~= nil
				or res:find("opcode encoding", 1, true) ~= nil
				or res:find("custom VM", 1, true) ~= nil
				or res:find("Failed to decompile", 1, true) ~= nil
		end

		env.decompile = function(...)
			if typeof(decompile) ~= "function" then
				return "-- Unable to decompile this script: this executor does not have a built-in decompile function."
			end

			local ok, res = pcall(decompile, ...)

			if ok and type(res) == "string" and not isDecompileError(res) then
				return res
			end

			-- Some executors return an error string instead of raising (e.g. obfuscated/encoded bytecode)
			if ok then
				if res == nil then
					return "-- Unable to decompile this script: the built-in decompiler returned no source."
				end
				return "-- Unable to decompile this script, the built-in decompiler failed:\n-- " .. tostring(res)
			end
			return "-- Unable to decompile this script, the built-in decompiler raised an error:\n-- " .. tostring(res)
		end
		--[[if Main.Elevated then
